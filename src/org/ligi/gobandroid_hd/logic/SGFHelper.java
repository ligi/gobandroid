...
public class SGFHelper {
	...
	public static String game2sgf(GoGame game) {
		...
		if (game.getHandicap() > 0) {
			black_to_move = false; // white begins on a handicap game - not
									// black
			res += "AB";
			byte[][] handicap_arr = GoDefinitions.getHandicapArray(game
					.getBoardSize());
			if (handicap_arr != null)
				for (int handicap = 0; handicap < game.getHandicap(); handicap++)
					res += coords2SGFFragment(handicap_arr[handicap][0],
							handicap_arr[handicap][1]);
			res += "\n";
		}
		...
		for (int p = 0; ((p < sgf.length()) && (!break_pulled)); p++) {
			...
			if (!consuming_param)
				...
				case '[':
					...
					if ((game == null)
							&& (act_cmd.equals("AB") || act_cmd.equals("AW")
									|| act_cmd.equals("TR")
									|| act_cmd.equals("SQ")
				...
				case '(':
					...
					if (!consuming_param) {
						...
						// push the move we where to the stack to return here
						// after the variation
				...
				// consuming param
				switch (act_char) {
				case ']': // closing command parameter -> can process command
							// now
					if ((game != null) && (callback != null))
						callback.progress(p, sgf.length(), game.getActMove()
								.getMovePos());
					if (!escape) {
						...
						// if we have a minimum of 2 chars in param - could be
						// coords - so parse
						...
						if (act_cmd.equals("Black") || act_cmd.equals("B")
							...
							// if still no game open -> open one with default
							// size
							if (game == null) {
								game = new GoGame((byte) 19);
								var_vect.add(game.getActMove());
							...
							if (act_param.length() == 0)
								...
								if (game.isBlackToMove()
										&& ((act_cmd.equals("W") || (act_cmd
												.equals("White")))))
									game.pass();
								else if ((!game.isBlackToMove())
										&& ((act_cmd.equals("B") || (act_cmd
												.equals("Black")))))
									game.pass();
						...
						if (act_cmd.equals("AddBlack") || act_cmd.equals("AB")
							...
							if (game == null) { // create a game if it is not
												// there yet
								game = new GoGame((byte) 19);
								var_vect.add(game.getActMove());
							}
