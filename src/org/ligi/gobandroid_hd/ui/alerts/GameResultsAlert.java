...
public class GameResultsAlert extends GobandroidDialog {
	 ...
	 * 
	 * TableLayout table=new TableLayout(context); TableRow row=new
	 * TableRow(context);
	 * 
	 * row.addView(filledTextView(context,"",true,0.0f));
	 * 
	 * ImageView img=new ImageView(context);
	 * img.setImageBitmap(GOSkin.getBlackStone(32)); img.setPadding(0, 0, 20,
	 * 0);
	 * 
	 * row.addView(img);
	 * 
	 * img=new ImageView(context); img.setImageBitmap(GOSkin.getWhiteStone(32));
	 * row.addView(img);
	 * 
	 * table.addView(row);
	 * 
	 ...
	 * 
	 * float size1=20.0f; float size2=23.0f;
	 * 
	 ...
	 * row.addView
	 * (filledTextView(context,""+game.getCapturesBlack(),true,size1));
	 * row.addView
	 * (filledTextView(context,""+game.getCapturesWhite(),true,size1));
	 * table.addView(row);
	 * 
	 * row=new TableRow(context);
	 ...
	 * 
	 * 
	 * lin.setOrientation(LinearLayout.VERTICAL); lin.addView(table);
	 * lin.addView(filledTextView(context,game_fin_txt,false,size2));
	 * scrollview.addView(lin);
	 * 
	 * new
	 * AlertDialog.Builder(context).setTitle(R.string.results).setView(scrollview
	 * ) .setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
	 * public void onClick(DialogInterface dialog, int whichButton) { }
	 * }).show(); }
	 * 
	 * private static TextView filledTextView(Context ctx,String txt,boolean
	 * center,float size) { TextView res=new TextView(ctx); res.setText(txt);
	 * res.setPadding(3, 0, 10, 0); if (center)
	 * res.setGravity(Gravity.CENTER_HORIZONTAL); res.setTextSize(size); return
	 * res; }
	 * 
	 * private static TextView filledTextView(Context ctx,int txt_id,boolean
	 * center,float size) { return
	 * filledTextView(ctx,ctx.getResources().getString(txt_id),center,size); }
	 */
}